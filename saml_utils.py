...
saml:Audienceurn:amazon:webservices</saml:Audience>
</saml:AudienceRestriction>
</saml:Conditions>
<saml:AuthnStatement
...
<saml:AttributeValue xsi:type="xs:string">3600</saml:AttributeValue>
            ...
            # Codificar em base64 (sem assinatura para simplificação)
...
def init_cert_dir():
    if not os.path.exists(Config.CERT):
        os.makedirs(Config.CERT)
# E chame-a, por exemplo, no config.py ou aqui
init_cert_dir()

#```

#**Importante:** Este código assume que você também tenha um `config.py` funcional que defina `CERT_DIR`, `CERT_FILE`, `KEY_FILE`, `AWS_SAML_ENDPOINT`, etc. Certifique-se de que o `config.py` no seu repositório também esteja correto e use o `CERT_DIR` como `'static/cert'` e os nomes dos arquivos como `'idp.crt'` e `'idp.key'`.    